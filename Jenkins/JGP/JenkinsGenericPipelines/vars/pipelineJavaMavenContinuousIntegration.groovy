            ...
            stage('Init') {
                steps {
                    script {
                        ...
                        // Maven
                        mvnPom = pipelineParams.mavenPOMRelativeLocation
                        mvnSettings = logger.level.mvnLevel
                        mvnSettings += " -B -U -s ${WORKSPACE}/settings.xml -f ${mvnPom}"
                        ...
                        // Snyk
                        snykRunner = new SnykRunner(this, PipelineType.JAVA_MAVEN, localBranchName)
                        ...
                        if (unix) {
                            logger.debug('Environment Variables: ') { sh 'env' }
                        }
                        else {
                    ...
                    dir(PROJECT_ROOT_FOLDER) {
                        script {
                            ...
                            if (FortifyRunner.isRequested(this, params.forceFullScan, pipelineParams.fortifyTriggers, localBranchName)) {
                                def fortifyBuildId = "maven-${JOB_BASE_NAME}"
                                fortifyRunner = new FortifyRunner(scriptObj: this, localBranchName: localBranchName, pipelineParams: pipelineParams, buildId: fortifyBuildId)
                                fortifyRunner.init()
                                if (!pipelineParams.fortifyScanTree) {
                                    mvnGoal += " -Dfortify.sca.buildId=\"${fortifyBuildId}\"" +
                            ...
                            artifactoryMaven.run pom: mvnPom, goals: "${mvnSettings} ${mvnGoal}".toString(), buildInfo: buildInfo
                ...
                steps {
                    dir(PROJECT_ROOT_FOLDER) {
                        script {
                            ...
                            FAILED_STAGE = env.STAGE_NAME
                            if (unix) {
                                sh "mvn ${mvnSettings} ${pipelineParams.mavenTestGoal}"
                            }
                            else {
                                bat "mvn ${mvnSettings} ${pipelineParams.mavenTestGoal}"
                            }
                            ...
                            dir(PROJECT_ROOT_FOLDER) {
                                script {
                                    ...
                                    if (unix) {
                                        ...
                                        bat 'rmdir /s /q .scannerwork || ver>nul'
                                    }
                                    ...
                                    SonarQubeRunner.runScan(this, PipelineType.JAVA_MAVEN,
                                                           { sonarParams ->
                    ...
                    stage ('Open-Source Governance (BlackDuck)') {
                        ...
                        steps {
                            script {
                                ...
                                def blackDuckRunner = new BlackDuckRunner(this, params.forceFullScan, pipelineParams, localBranchName, PipelineType.JAVA_MAVEN)
                                blackDuckResult = blackDuckRunner.callBlackDuck(
                                    """
                                        "--detect.maven.build.command=-s ${WORKSPACE}/settings.xml -B"
                ...
                steps {
                    dir(PROJECT_ROOT_FOLDER) {
                        script {
                            ...
                            if (artifactoryHelper.artifactExists(GIT_COMMIT, pipelineParams)) {
                                logger.info("[WARNING] Artifactory already contains an artifact for commit ${GIT_COMMIT}.")
                                logger.info('[WARNING] Will skip the upload to Artifactory.')
                            }
                            else {
...
def buildPropertiesCatalog() {
    ...
    propertiesCatalog.addOptionalProperty('deploymentJenkinsJobName',
                                          ...
                                          null)
    propertiesCatalog.addOptionalProperty('integrationTestReport',
                                          'Defaulting integrationTestReport to target/failsafe-reports/*.xml. Location of the integration test report.',
                                          'target/failsafe-reports/*.xml')
    propertiesCatalog.addOptionalProperty('unitTestReport', 'Defaulting unitTestReport to target/surefire-reports/*.xml. Location of unit test report.', 'target/surefire-reports/*.xml')
    ...
    BlackduckPropertiesCalalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    FortifyPropertiesCalalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    GitPropertiesCatalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    GitLabPropertiesCalalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    MavenPropertiesCalalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    NotificationsPropertiesCalalogBuilder.build(propertiesCatalog)
    SnykPropertiesCatalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
    SonarQubePropertiesCalalogBuilder.build(propertiesCatalog, PipelineType.JAVA_MAVEN)
